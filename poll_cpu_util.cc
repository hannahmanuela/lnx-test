...
int main() {
    ...
    attr.sched_runtime = 50 * NSEC_PER_MSEC;
    ...
    cout << "took: " << time_since_start(start) << "ms" << endl;

    // struct sched_attr attr;
    ret = syscall(SYS_sched_getattr, getpid(), &attr, sizeof(attr), 0);
    if (ret < 0) {
        perror("ERROR: sched_getattr");
    }
    attr.sched_runtime = 8 * NSEC_PER_MSEC;
    ret = syscall(SYS_sched_setattr, getpid(), &attr, 0);
    if (ret < 0) {
        perror("ERROR: sched_setattr");
    }
